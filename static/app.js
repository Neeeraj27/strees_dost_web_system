...
let mentorCounterTimer = null;
let mentorCount = 0;
let fakeCompetitorTimer = null;
let questionTimerTicker = null;
let questionTimerDeadline = null;
let confirmSubmitStage = 0;
let pendingConfirmResolver = null;

const QUESTION_TIME_LIMIT_MS = 60 * 1000;
const QUESTION_PANIC_THRESHOLD_MS = 20 * 1000;
const QUESTION_BLINK_THRESHOLD_MS = 10 * 1000;
...
const CLIENT_FALLBACK_QUESTIONS = [
  {
    ...
    subject: "Physics",
    difficulty: "Easy",
    question_html: "<p>A body accelerates at 2 m/s² for 5 s from rest. Final velocity?</p>",
    options: [
      { label: "A", text: "5 m/s" },
      { label: "B", text: "10 m/s" },
      { label: "C", text: "12 m/s" },
  ...
  },
  {
    ...
    subject: "Math",
    difficulty: "Easy",
    question_html: "<p>For x² - 5x + 6 = 0, sum of roots equals?</p>",
    options: [
      { label: "A", text: "2" },
      { label: "B", text: "3" },
      { label: "C", text: "5" },
  ...
  },
  {
    ...
    subject: "Math",
    difficulty: "Easy",
    question_html: "<p>sin 30° equals:</p>",
    options: [
      { label: "A", text: "1/2" },
...
async function postJSON(url, body, opts) {
  ...
  try {
    ...
    if (err && err.name === "AbortError") {
      throw new Error("Request timed out. Proceeding with local test stage.");
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
function showStage(name, message) {
  if (name !== "popups") {
    completeSubmitConfirmation(false);
  }
...
function setMentorCounter(count) {
  mentorCount = Math.max(1, Math.floor(Number(count) || 1));
  if (mentorsWatching) {
    mentorsWatching.textContent = `Mentors watching: ${mentorCount}`;
    mentorsWatching.classList.add("bump");
    setTimeout(() => mentorsWatching.classList.remove("bump"), 220);
  }
}

function scheduleMentorCounterTick() {
  clearTimeout(mentorCounterTimer);
  mentorCounterTimer = setTimeout(() => {
    const delta = randomInt(-2, 3);
    setMentorCounter(Math.max(1, mentorCount + delta));
    scheduleMentorCounterTick();
  }, randomInt(4000, 9000));
}

function startMentorCounter() {
  setMentorCounter(randomInt(6, 18));
  scheduleMentorCounterTick();
}

function stopMentorCounter() {
  clearTimeout(mentorCounterTimer);
  mentorCounterTimer = null;
  if (mentorsWatching) mentorsWatching.textContent = "Mentors watching: —";
}

function renderQuestionTimer(msLeft) {
  ...
  const prefix =
    safeMs <= QUESTION_BLINK_THRESHOLD_MS
      ? "Critical"
      : safeMs <= QUESTION_PANIC_THRESHOLD_MS
        ? "Hurry"
        : "Time left";
  questionTimerEl.textContent = `${prefix}: ${min}:${sec}`;
  questionTimerEl.classList.toggle("panic", safeMs <= QUESTION_PANIC_THRESHOLD_MS);
  questionTimerEl.classList.toggle("blink", safeMs <= QUESTION_BLINK_THRESHOLD_MS);
}

function stopQuestionTimer() {
  clearInterval(questionTimerTicker);
  questionTimerTicker = null;
  questionTimerDeadline = null;
  if (questionTimerEl) {
    questionTimerEl.textContent = "Time left: --:--";
    questionTimerEl.classList.remove("panic", "blink");
  }
}
...
function startQuestionTimer() {
  ...
  clearInterval(questionTimerTicker);
  questionTimerDeadline = Date.now() + QUESTION_TIME_LIMIT_MS;
  renderQuestionTimer(QUESTION_TIME_LIMIT_MS);
  questionTimerTicker = setInterval(() => {
    if (!questionTimerDeadline) return;
    const msLeft = questionTimerDeadline - Date.now();
    renderQuestionTimer(msLeft);
    if (msLeft <= 0) {
      stopQuestionTimer();
      setTestHint("Time is up for this question. Decide fast and move on.");
    }
  }, 250);
}

function queueFakeCompetitorMessage() {
  if (!testQuestions.length) return;
  const questionNum = randomInt(1, testQuestions.length);
  const names = ["Ananya", "Rohan", "Meera", "Arjun", "Kavya", "Ishaan", "Priya", "Neel"];
  const tones = [
    "just locked Question",
    "submitted Question",
    "finished Question",
    "moved past Question",
  ];
  const tails = [
    ...
    "Pace is picking up.",
    "Leaderboard is shifting fast.",
    "Stay sharp now.",
  ];
  const name = names[randomInt(0, names.length - 1)];
  const tone = tones[randomInt(0, tones.length - 1)];
  const tail = tails[randomInt(0, tails.length - 1)];
  const line = `${name} ${tone} ${questionNum}. ${tail}`;
  if (competitorFeed) {
    competitorFeed.textContent = `Live arena: ${line}`;
    competitorFeed.classList.add("flash");
    setTimeout(() => competitorFeed.classList.remove("flash"), 620);
  }
  enqueuePopup({
    type: "comparison",
    message: line,
    ttl: randomInt(3200, 5200),
  });
}

function scheduleFakeCompetitorMessage() {
  clearTimeout(fakeCompetitorTimer);
  fakeCompetitorTimer = setTimeout(() => {
    queueFakeCompetitorMessage();
    scheduleFakeCompetitorMessage();
  }, randomInt(9000, 17000));
}

function startFakeCompetitorMessages() {
  if (competitorFeed) competitorFeed.textContent = "Live arena: Students are submitting answers in real time.";
  scheduleFakeCompetitorMessage();
}

function stopFakeCompetitorMessages() {
  clearTimeout(fakeCompetitorTimer);
  fakeCompetitorTimer = null;
  if (competitorFeed) competitorFeed.textContent = "Live arena feed will appear here.";
}
...
function hideSubmitConfirmModal() {
  ...
  confirmSubmitStage = 0;
}

function showSubmitConfirmModal() {
  if (!submitConfirmModal || !btnConfirmPrimary || !btnConfirmCancel || !confirmTitle || !confirmMessage) {
    return Promise.resolve(window.confirm("Are you absolutely sure?"));
  }
  submitConfirmModal.hidden = false;
  document.body.style.overflow = "hidden";
  confirmSubmitStage = 1;
  confirmTitle.textContent = "Lock your answer?";
  confirmMessage.textContent = "This saves your choice for this question.";
  btnConfirmPrimary.textContent = "Submit answer";
...
function completeSubmitConfirmation(confirmed) {
  if (pendingConfirmResolver) {
    pendingConfirmResolver(Boolean(confirmed));
    pendingConfirmResolver = null;
  }
  hideSubmitConfirmModal();
}

async function submitCurrentQuestionWithConfirmation() {
  if (!testQuestions.length) {
    setTestHint("Load questions first.");
    return;
  }
  const confirmed = await showSubmitConfirmModal();
  if (!confirmed) return;
...
async function skipRemainingQuestions() {
  ...
  try {
    ...
    await postJSON(`/session/${sessionId}/complete`, {}, { timeoutMs: 8000 });
    await handleCompletion();
  } catch (err) {
    // If backend completion is slow/hung, still move forward to trigger test UI.
    log("skip_warning", err.message);
    await handleCompletion();
  }
}
...
btnSubmitQuestion?.addEventListener("click", submitCurrentQuestionWithConfirmation);
btnConfirmCancel?.addEventListener("click", () => completeSubmitConfirmation(false));
btnConfirmPrimary?.addEventListener("click", () => {
  if (!confirmTitle || !confirmMessage || !btnConfirmPrimary) {
    completeSubmitConfirmation(true);
    return;
  }
  if (confirmSubmitStage === 1) {
    confirmSubmitStage = 2;
    confirmTitle.textContent = "Final confirmation";
    confirmMessage.textContent = "Are you absolutely sure? This action cannot be undone.";
    btnConfirmPrimary.textContent = "Yes, lock it";
    return;
  }
  completeSubmitConfirmation(true);
});
submitConfirmModal?.addEventListener("click", (evt) => {
  if (evt.target === submitConfirmModal) completeSubmitConfirmation(false);
});
